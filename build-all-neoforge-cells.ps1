# build-all-neoforge-cells.ps1 -- walk the pre-26 NeoForge/<v> cells, cog-gen + gradlew clean build each,
# copy the shaded jar into dist/ with a line-keyed name (Chunksmith-NeoForge-<modver>+mc<v>.jar).
# The 26 line is built by build-all-neoforge.ps1 (different -P matrix) and is NOT walked here.
#
# Usage:
#   pwsh build-all-neoforge-cells.ps1                # build every pre-26 NeoForge cell
#   pwsh build-all-neoforge-cells.ps1 -Only 1.21.8   # build ONE cell (respects the 45s shell cap)
param([string]$Only)
