 ...
 * <p>
 * 1.20.1 ADAPTATION: on 1.20.1 the write radius is a primitive field {@code writeRadiusCutoff}
 * (the 26.x {@code generatingStep.blockStateWriteRadius()} does not exist), and the generation
 * step is a {@code ChunkStatus generatingStatus} field (not a {@code ChunkStep generatingStep}).
 * The far-write log call is still {@code Util.logAndPauseIfInIde(String)} inside
 * {@code WorldGenRegion.ensureCanWrite(BlockPos)}, byte-stable on 1.20.1, so the @Redirect target
 * is identical. {@code getCenter()} and the {@code currentlyGenerating} supplier are unchanged.
 */
...
public abstract class WorldGenRegionMixin {
    ...
    private void chunky$captureFarWrite(final String message, final BlockPos pos) {
        ...
        try {
            ...
            final String step = String.valueOf(this.generatingStatus);
            ...
            WorldgenOverreachReporter.get().record(
                    ...
                    pos.getY(), this.writeRadiusCutoff);
