 ...
 * moves it -- and a same-millisecond collision is unreachable because the server will not index a region
 * until it has been UNTOUCHED for {@link CsLodStoreScan#SETTLE_MILLIS} (10 s). It detects CHANGE, not
 * corruption: bad bytes in transit are HTTP's and TCP's problem.
 *
 ...
 * says "unchanged" about a region that changed.
 *
 * <p>The client never recomputes this -- it cannot, the mtime of its own copy is when the CLIENT wrote it.
 * The token is OPAQUE to it (see {@code CsLodManifest}).
 */
