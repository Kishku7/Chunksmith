 ...
 * version-specific.
 *
 * <p>Two transports, one protocol: in-band ({@link #CHANNEL}) shares the game connection
 * and must yield to gameplay; the HTTP backchannel at {@link #httpPort game port + 1} is
 * the fast path.
 *
 * <p>Authentication: the in-band handshake rides a connection the player has already
 * authenticated with Mojang, so the server issues a short-lived random token bound to
 * (uuid, ip, expiry) which the client presents to the HTTP endpoint. A UUID or a name
 * proves nothing on its own: both are public.
 */
public final class CsLodProtocol {
     ...
     *
     * <p><b>v2 (3.1.0-beta-4).</b> {@code S2C_INDEX}'s layout is unchanged from v1; the
     * meaning of its {@code long} hash is not. v1 was a CRC32 of the region file's contents
     * computed independently by both ends. See {@link CsLodRegionHash} for what that cost a
     * live production server. v2 is an opaque freshness token derived from the server's
     * (mtime, size), which the client remembers rather than recomputes. A v1 client would
     * find every region stale every time and re-download the whole in-radius store every
     * five seconds, so the handshake refuses instead: v1 -> v2 still gets a v2 {@code
     * S2C_HELLO} with {@code storeAvailable=false} so it can name the mismatch, and v2 ->
    ...
    /**
     * Max region entries in a single {@code S2C_INDEX}. The server indexes only inside the
     * client's clamped radius (MAX_RADIUS_BLOCKS 16384 -> ceil(16384/512)*2+1 = 65 per side
     * -> ~4225 regions) and a per-request fetch is itself capped at 4096; 65536 is ~15x
     * headroom.
     */
    public static final int MAX_INDEX_REGIONS = 65536;

    /**
     * Max byte length of one in-band {@code S2C_CHUNK} slice. The sender drips fixed 24 KiB
     * slices (CsLodInBandSender.SLICE_BYTES = 24 * 1024); 1 MiB is ~42x headroom and still
     * refuses a multi-GB length.
     */
    public static final int MAX_SLICE_BYTES = 1 << 20;

    /**
     * Max entries in one CSLOD palette (block or biome). Palette indices are serialized as
     * 1 byte (palette &lt;= 256) or 2 bytes ({@code CsLodCodec.indexWidth}), so 65536 is
     * the exact ceiling.
     */
    public static final int MAX_PALETTE_SIZE = 65536;

    /**
     * Max sections in one CSLOD record. The count rides a single unsigned byte, so it is
     * already bounded to 255, and the height limit (&lt;= 4064 blocks -> 254 sections) sits
     * under that; 256 guards a width change.
     */
    public static final int MAX_SECTIONS = 256;

    /**
     * Max byte length of one stored CSLOD record. Uncompressed worst case ~254 sections x
     * ~12.4 KiB + palettes (~6 MiB), Deflate-compressed on disk, so 32 MiB is ~5x headroom.
     * Bounds the {@code new byte[length]} in the region store, where {@code length} comes
     * from a region-file header whose bytes may have been streamed in-band from an
     * untrusted server.
     */
     ...
     *
     * <p>Deliberately not "send me the index": the index is the expensive thing. The client
     * pays for a real index only when {@link #S2C_SUMMARY}'s two numbers disagree.
     */
    public static final byte C2S_REQUEST_SUMMARY = 5;

    /**
     * S2C: the region index folded to (count, aggregate), the answer to {@link
     * #C2S_REQUEST_SUMMARY} (v2). 34 bytes. See {@link CsLodSummary} for why the aggregate
     * is an order-independent XOR.
     */
     ...
     * {@code config/chunksmith-lod.properties} on their client, which on a dedicated server
     * does not exist. So the server forwards and the client reads, writes and prints its
     * own reply. Purely additive, so no {@link #VERSION} bump: an older client drops the
     * unknown id, which is why the server checks {@code CsLodServerNet.hasLodClient} first.
     */
    ...
    /**
     * Returns the backchannel port, derived from the game port. Game on 25565 -> HTTP on
     * 25566. This is only the default; an operator may name one (mod_support #19), see
     * {@link #httpPort(int, int)}.
     *
    ...
    /**
     * Resolves the backchannel port an operator actually gets. Their configured port if
     * they named one, otherwise {@link #httpPort(int) the derived one}. A configured port
     * that collides with the game port is refused rather than honoured, since binding it
     * cannot succeed anyway, and refusing lets the caller report a cause instead of an
     * anonymous bind failure.
     *
