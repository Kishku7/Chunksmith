...
a server is strongly discouraged and, from 3.18.1, **the server tells you so in red at startup**. It
competes for the same CPU Chunksmith is pre-generating on and duplicates the LOD your players are
already being sent; a server-side Distant Horizons also fills the console with
`[SharedApi] No DH level provided ...` warnings. Nothing is gained. If LOD is misbehaving on your
server, this is the first thing to check.
...
## Commands you run, and commands your client runs

Chunksmith 4 splits its commands by who they belong to.

**`/cs ...` is the server's.** Everything above -- `world`, `radius`, `start`, `pause`, `set` -- plus
`/cs lod status` and `/cs lod token <player>` for the LOD store the server keeps. It needs operator
permission, same as it always did.

**`/csclient ...` is yours.** It runs on your own machine, at any permission level, against your own
copy of the LOD data:

```
/csclient status          # what you have, where it is, and what is arriving
/csclient set             # your client settings, listed
/csclient reset           # throw your copy away and pull it again
/csclient inject_voxy     # replay your store into Voxy
/csclient inject_dh       # replay your store into Distant Horizons
```

Because it is a real client command, it works even on a server that has never heard of Chunksmith --
which is exactly when you want to ask why no distant terrain is showing up.

`/csclient reset` is the one to reach for when your horizon looks wrong. It deletes your copy for that
server and pulls it fresh, without a relog. It will tell you one thing it cannot do: your renderer
keeps its own database, and Chunksmith does not touch it, so if the server rebuilt its world you may
need to clear Distant Horizons or Voxy separately.

**`/cslod` is gone.** If you type it on a Chunksmith 4 server with an older client, it will tell you
to update. If your client is current, it simply does not exist any more.
